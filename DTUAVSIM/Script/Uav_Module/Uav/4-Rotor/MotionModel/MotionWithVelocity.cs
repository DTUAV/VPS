...
namespace DTUAV.UAV_Module.Quadrotor
{
    public class MotionWithVelocity
    {
            ...
            public MotionWithVelocity(Rigidbody objectRigidbody, Vector3 targetVelocity, float maxAngle, float runningHz)
        {
            ...
            _currentGlobalPosition = TF.Unity2Ros(objectRigidbody.position);
            _currentLocalPosition = Vector3.zero;
            _currentVelocity = TF.Unity2Ros(objectRigidbody.velocity);
        ...
        void Run()
        {
            while (_isRun)
            {
                ...
                {
                    Vector3 targetGlobalVelocity = TF.Ros2Unity(_targetVelocity);
                    Vector3 currentLocalVelocity = TF.Global2Local(new Vector3(targetGlobalVelocity.z,0,-targetGlobalVelocity.x), _objectRigidbody.rotation.eulerAngles.y * Mathf.Deg2Rad);
                    _objectRigidbody.velocity = TF.Ros2Unity(_targetVelocity);
                    _currentGlobalPosition = TF.Unity2Ros(_objectRigidbody.position);
                    _currentLocalPosition = _currentGlobalPosition - _homePosition;
                    _currentVelocity = TF.Unity2Ros(_objectRigidbody.velocity);
