...
public class DroneBase : MonoBehaviour
{
	...
	bool init_object()
	{
		if (droneV2 == null || propellorBL == null || propellorBR == null || propellorFL == null || propellorFR == null)
		{
			Debug.Log("对象未初始化");
			return false;
		}
	...
	void updateBase()
	{
		...
		currentAngle = new Vector3(
			Mathf.LerpAngle(currentAngle.x, targetAngle.x, Time.fixedDeltaTime * speed_k),
			Mathf.LerpAngle(currentAngle.y, targetAngle.y, Time.fixedDeltaTime * speed_k),
			Mathf.LerpAngle(currentAngle.z, targetAngle.z, Time.fixedDeltaTime * speed_k)
		);

		// Set the rotation angle.
	droneV2.transform.eulerAngles = new Vector3 (currentAngle.x,currentAngle.y,currentAngle.z);
		droneV2.transform.localEulerAngles = new Vector3(droneV2.transform.localEulerAngles.x,ref_yaw,droneV2.transform.localEulerAngles.z);
	}
