...
namespace DTUAVCARS.DTMission
{
     ...
     * (2)define a class instance : private triangle_track _triangleTrack = new triangle_track(Vector3 center,float width,float lendth,float heigh,bool isLoop,float positionError,int planeId)
     * (3)check if configure finnish:bool isFinnish = _triangleTrack.GetIsGetNewPose()
     * (4)update && get data:1)Vector3 targetPosition = _triangleTrack.GetTargetPosition(Vector3 currentPosition)
     */
    public class triangle_track
    {
        private Vector3 _center;   //The Center Position of Triangle Track
        private float _width;     //The Width of Triangle ---Y
        private float _lendth;   //The Lendth of Triangle ---X
        private float _heigh;   //The Heigh of Triangle ---Z
        private bool _isLoop;   //If loop the Triangle Track
                ...
                case 0://X-Y
                    {
                        ...
                        _targetPositions[1] = _center + new Vector3(0, 0.5f * _width, 0);
                ...
                case 1: //X-Z
                    {
                        if (_center.z - 0.5f * _heigh > 0.5f)
                        {
                            ...
                            _targetPositions[1] = _center + new Vector3(0, 0, 0.5f * _heigh);
        ...
        public triangle_track(Vector3 center, float width, float lendth, float heigh, bool isLoop, float positionError, int planeId)
        {
            _center = center;
            _width = width;
            _lendth = lendth;
            ...
            _planeId = planeId;
            _nowPositionId = 0;
            _targetPositions = new List<Vector3>(3);
            for (int i = 0; i < 3; i++)
            {
