...
{
    public class SocketClient2 : MonoBehaviour, LCM.LCM.LCMSubscriber
    {
            ...
            while (!endFlag)
            {
                try
                {
                    ...
                    if (socketSend.Receive(buf, 0, 2, SocketFlags.None) == 2)
                    {
                        if (buf[0] == '\n' && buf[1] == '\n')
                        {
                            byte[] bufDataSize = new byte[32];//接收32位//
        ...
        public void MessageReceived(LCM.LCM.LCM lcm, string channel, LCMDataInputStream ins)
        {
            if (channel == IotMessageSubName)
            {
                ...
                string dataLenStr = Convert.ToString(dataLen, 2).PadLeft(32, '0');//32位 字符串大小32位
