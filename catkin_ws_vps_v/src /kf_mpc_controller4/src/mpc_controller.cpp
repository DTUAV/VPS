...
mpc_controller::mpc_controller()
{
  ...
  n.getParam("system_running_control_sub_topic",_system_running_control_sub_topic);
  ...
  _reset_object_sub = n.subscribe(_reset_object_sub_topic,1,&mpc_controller::reset_object_sub_cb,this);
  ...
  _system_running_control_sub = n.subscribe(_system_running_control_sub_topic,1,&mpc_controller::system_running_control_sub_cb,this);
  ...
  _system_running_id = 0;

}
void mpc_controller::reset_object_sub_cb(const std_msgs::Float64MultiArrayConstPtr& msg)
{
  _reset_position_x = msg.get()->data.at(0);
  _reset_position_y = msg.get()->data.at(1);
  _reset_position_z = msg.get()->data.at(2);
}

void mpc_controller::system_running_control_sub_cb(const std_msgs::Int32ConstPtr &msg)
{
  _system_running_id = msg.get()->data;
}
void *mpc_controller::controller_running(void *args)
{
  ...
  while(ros::ok())
  {
    if(mpc_controller_node->_system_running_id == 1||mpc_controller_node->_system_running_id == 2)
    {
...
{
  //  if(_system_running_id == 1||_system_running_id == 2)
  //  {
  ...
  //     for(int i = 0; i<_state_dim*_predict_window; i = i+3)
  //     {
  //         temp(i,0) = _reset_position_x;
  //         temp(i+1,0) = _reset_position_y;
  //         temp(i+2,0) = _reset_position_z;
  //     }
  //     Update_Target(temp);
  //  }
}
