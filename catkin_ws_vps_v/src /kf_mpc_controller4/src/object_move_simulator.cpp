...
object_move_simulator::object_move_simulator(int state_dim, int observe_dim, int input_dim)
{
  ...
  n.getParam("start_simulator_pub_topic",_start_simulator_pub_topic);
  ...
  _reset_object_sub = n.subscribe(_reset_object_sub_topic,1,&object_move_simulator::reset_object_sub_cb,this);

  _sensor_data_pub = n.advertise<geometry_msgs::PoseStamped>(_sensor_data_pub_topic,1);
  _start_simulator_pub = n.advertise<std_msgs::Bool>(_start_simulator_pub_topic,1);
...
void object_move_simulator::reset_object_sub_cb(const std_msgs::Float64MultiArrayConstPtr &msg)
{
  _isResetObject = true;
  if(msg.get()->data.size()==_state_dim)
  {
    Eigen::MatrixXf X(_state_dim,1);
    for(int i=0;i<_state_dim;i++)
    {
      X(i,0) = msg.get()->data.at(i);
    }
    ROS_INFO("Simulator----Reset Object....");
    if(ResetObject(X))
    {
      ROS_INFO("Simulator----Reset Object Sucessfully!!!!!");
      std_msgs::Bool start_simulation_msg;
      start_simulation_msg.data = true;
      _start_simulator_pub.publish(start_simulation_msg);
    }
    else
    {
      ROS_INFO("Simulator----Reset Object Fail!!!!!");
    }
  }
  else
  {
...
bool object_move_simulator::ResetObject(Eigen::MatrixXf X)
{
  return(_object->reset_Object_State(X));
}
