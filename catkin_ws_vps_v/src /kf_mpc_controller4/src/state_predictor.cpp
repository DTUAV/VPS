...
state_predictor::state_predictor()
{
  ...
  n.getParam("predictor_x_pub_topic",_predictor_x_pub_topic);
  n.getParam("predictor_y_pub_topic",_predictor_y_pub_topic);
  n.getParam("predictor_z_pub_topic",_predictor_z_pub_topic);

  n.getParam("predictor_cur_position_pub_topic",_predictor_cur_position_pub_topic);
  ...
  _predictor_x_pub = n.advertise<std_msgs::Float64MultiArray>(_predictor_x_pub_topic,1);
  _predictor_y_pub = n.advertise<std_msgs::Float64MultiArray>(_predictor_y_pub_topic,1);
  _predictor_z_pub = n.advertise<std_msgs::Float64MultiArray>(_predictor_z_pub_topic,1);

  _predictor_cur_position_pub = n.advertise<geometry_msgs::PoseStamped>(_predictor_cur_position_pub_topic,1);

  _reset_object_sub = n.subscribe(_reset_object_sub_topic,1,&state_predictor::reset_object_sub_cb,this);
...
void state_predictor::reset_object_sub_cb(const std_msgs::Float64MultiArrayConstPtr& msg)
{
   _isResetObject = true;
   {
...
void state_predictor::system_state_sub_cb(const dt_message_package::object_move_stateConstPtr &msg)
{
  if(!_isResetObject)
  {
   ...
   // std::cout<<"dddddd_ffffff"<<std::endl;
  }
}
...
void *state_predictor::predictor_running(void *args)
{
  ...
  std_msgs::Float64MultiArray predictor_x_msg;
  std_msgs::Float64MultiArray predictor_y_msg;
  std_msgs::Float64MultiArray predictor_z_msg;

  predictor_x_msg.data.resize(predictor->_predict_window);
  predictor_y_msg.data.resize(predictor->_predict_window);
  predictor_z_msg.data.resize(predictor->_predict_window);

  geometry_msgs::PoseStamped predictor_local_position_msg;

  while(ros::ok())
  {
    if(!predictor->_isResetObject)
    {
      if(predictor->_cicl_queue_t.at(predictor->_cicl_queue_t.size()-1)!=0)
      {
        ...
        for(int i = 0; i< pdt_state_x_stu.states.size();i++)
        {
          ...
          predictor_x_msg.data.at(i) = pdt_state_x_stu.states.at(i);
          predictor_y_msg.data.at(i) = pdt_state_y_stu.states.at(i);
          predictor_z_msg.data.at(i) = pdt_state_z_stu.states.at(i);
        }
        ...
        predictor_local_position_msg.pose.position.x = current_position_x;
        predictor_local_position_msg.pose.position.y = current_position_y;
        predictor_local_position_msg.pose.position.z = current_position_z;

        predictor->_predictor_cur_position_pub.publish(predictor_local_position_msg);

        predictor->_predictor_x_pub.publish(predictor_x_msg);
        predictor->_predictor_y_pub.publish(predictor_y_msg);
        predictor->_predictor_z_pub.publish(predictor_z_msg);
