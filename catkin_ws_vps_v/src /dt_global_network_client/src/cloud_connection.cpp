...
void iot_recv_process(string msg)
{
  ...
  if(msg != "")
  {
    ...
    {
      DTUAVCARS::IotMessage iot_msg;
      bool is_load = x2struct::X::loadjson(msg,iot_msg,false);
      if(is_load)
      {
        cloud_common::socket_data_packet_stamp_msg ros_iot_msg;
...
void iot_msg_cb(const cloud_common::socket_data_packet_stamp_msg::ConstPtr& msg)
{
  DTUAVCARS::IotMessage iot_msg;
...
int main(int argc, char *argv[])
{
  ...
  iot_msg_pub = nh.advertise<cloud_common::socket_data_packet_stamp_msg>(ros_pub_topic, 10);
